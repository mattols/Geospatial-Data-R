...
title("Study Region Location")
box()
plot(ext(lux), border="firebrick", lwd=1.5, add=T)
map.text("world", c("Belgium", "Germany$"), add=T)
# map 2
plot(lux, "POP", col=blues9, main = "Population by Canton", mar=c(2,1,2,5), plg=list(cex=0.5))
map("world", c("Belgium", "Germany", "Luxembourg"), col='grey40', lty=3, lwd=2, add=T)
map.text("world", "Luxembourg", add=T, col="firebrick", cex=1.2)
text(x=6.35, y=50.1, "Germany", cex=0.8, col='grey30')
```

To interpret this:

* the `par()` function allows you to set graphical parameters, the mfrow argument sets the number of c(row, column) plots
* the `mar` argument within the plot functions access the `par(mar=...)` parameter
* `box()` draws the rectangular plot box, `axis`, `text`, and `title` components can also be added
* the `maps` library can be used to quickly visualize country and state borders alongside other spatial data (including text)
* use `?plot` (from terra) to learn more about the different parameters; common arguments include line width (`lwd=`), line type (`lty=`), size (`cex=`), `plg` to access plot axis parameters, and `add=T` to continue adding data to the current plot

Don't feel the need to spend too much time trying to make a beautiful map. You'll learn more as you go and discover other workflows and packages for plotting. 
...
2. (2 points) Next, you will need to revisit your code for task 6. Summarize the "UTSNTL_ALL_2020_2022_Daily_Wide.csv" table to only show the max snow depth and station name for each station during the 2021-22 winter season. Create a join between your summarized max snow data and the table containing longitude, latitude, and elevation. Be sure that the join is completely successful! Make this into a SpatialVector object. Create a basic plot showing max snow depth varying by point size for the various stations, then add a second point for each station.....
