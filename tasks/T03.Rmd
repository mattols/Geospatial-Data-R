...
### Data Formatting and Variable Relationships
...
#### 5. Basic Data Relationships

Interpreting the strength and direction of relationships between variables is very important in any analysis. Although a simple scatterplot can give you a good initial idea of the relationship between two variables, it is necessary to have statistical values to support any observations or hypothesis testing. 

This class is not a substitute for a statistics class, and in reality, you should have already taken a stats course or enroll in one soon.
...
```r 
fit <- lm(y ~ x1 , data=mydata) # can use simple or multiple linear regression (y ~ x1 + x2)
summary(fit) # view model summary
confint(fit) # 95% CIs for coefficients 
```

The general steps and useful functions for linear models include:
- Fit linear regression models with the lm() function
- Specify formula like y ~ x1 
...
Model residuals, predictions, diagnostics, and additional details can be extracted with extractor functions. For example, statistical tests like linear models produce p-values to test null hypotheses. P-values below 0.05 are often considered statistically significant:
...
T-tests, chi-squared, ANOVA, and correlation tests can be carried out for hypothesis testing. T-tests compare the distribution of two groups while ANOVA compares the distribution of more than two groups. Chi-squared tests allow analyzing relationships between categorical variables ANOVA is for factor-based group comparisons, in which post-hoc comparison tests, like `TukeyHSD`, can help determine which groups differ. 

For group differences and hypothesis testing, R provides some of the following functions:
...
These tests make assumptions like normality and equal variance that should be checked. If violated, non-parametric tests may be better. You can read more about these tests [here](https://www.voxco.com/blog/anova-vs-t-test-with-a-comparison-chart/) or elsewhere online.

Correlation can be calculated using standard methods like Pearson's r (for linear) or Spearman (for monotonic) to characterize association between variables: 

```r
cor.test(x, y) # correlation test 
```

While this is not a statistics class, we will be using statistical tests and models to analyze data regularly. If you need it, read up on some of the definitions and concepts on your own time.
...
4. With the emails vector, write a function to identify only valid email addresses containing exactly one @ symbol and at least one period using regular expression. *(can use `grep()` or another)*

5. Using the phones vector, write a regular expression pattern to identify all valid 10-digit phone numbers in the phones vector, regardless of how they are formatted with parentheses, spaces, dots, etc. *(Tip: consider the format of different 10-digit numbers and account for optional non-digit characters, 3 digits, optional non-digits, 3 digits, optional non-digits, and 4 digits)* - advanced students should write a script to check to try and double check that their responses are valid.

6. Subset all valid phone numbers that contain the format XXX-XXX-XXXX and replace the "-" with "_" using `gsub` or other function.
...
8. How do density and snow depth vary over time? Create a single plot showing both snow depth and density as a function of time. *Note you can use the `lines()` or `points()` functions to add more to an existing scatter/line plot.* What day seems to mark the transition from clear to stormy weather?

9. Is there a correlation between snow water equivalent and precipitation accumulation? Describe the strength and direction of any relationship.

10. Create a new column called swe_perc showing the snow water equivalent as a percent of the median historical swe value for each day. Plot these values and describe how the swe values track with the median historical values? 

11. How much snow (*depth*) was gained or lost from the start to end of time period? How much snow water equivalent was gained or lost? 

12. Write a script to calculate the daily incremental change in both snow depth and SWE over the 30 day period. Sum both the gains and losses from each and compare these values to the values in the previous question. What might account for the differences?

13. Is there a relationship between air temperature and snow depth? Run a linear model, plot the results (*showing scatterplot and linear model*), and report the $R^{2}$ and p-value. Run a correlation test as well.

14. Create a new DataFrame that is a subset of this data and only include dates after the storms arrived (*i.e. keep days after new snow started accumulating*). Run the same tests as in the previous question and compare the statistical strength. How has the relationship changed at all?

15. Assess the relationship between snow density and temperature, as well as any other variables you think might relate to one another. Describe any patterns or relationships you notice in your initial analysis of this dataset. Advanced students should try multiple linear regression.
