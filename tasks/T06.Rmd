...
4. Use the results above to generate a histogram showing the counts of snotel sites in the top 5 for for snow depth over the three seasons. Which sites continually receive the deepest snow depth?
...
#   or see description in:
?pivot_longer


```

|     *Seasons and snow depth tips:*
...
#   subsetting can be achieved with filter(), arrange(), or the slice_max() function to combine several steps
```
...
Snow water equivalent (SWE) is an important metric used to assess the amount of water in the snowpack. In particular, the month of April can be critical for water managers to assess conditions for the coming year. 

Perform a join with the data above and  the "UTSNTL_ELEV.csv" file. Show code for your join and complete the following:

5. (2 points) Join the two dataframes and prove that the match was fully successful for all sites (all sites should exist and have a valid elevation value). Create a scatter plot that shows the max SWE (snow water equivalent) during the month of April as a function of elevation.

6. It is well known that precipitation amount can be enhanced at higher elevations due to orographic lift (air masses flowing over mountains). Utah's mountains benefit and often have an enhancement of 2-5 times the amount of precipitation. Assess the strength of the relationship above statistically and hypothesize as to why the relationship is not high? *i.e. what other variables might influence the amount of snow at different sites*
