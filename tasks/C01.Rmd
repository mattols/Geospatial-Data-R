...
#### Computational Solutions in Earth Science
...
*Note: Any samples with greater than 60% Quartz content can be labeled as `NA` or "Unkown".*
...
1) *(4 points)* Provide your code solution to classifying the rock sample data based on the simplified QAP diagram above. Remember, your solution does not need to be perfect.
...
5) *(2 points)* Based on your analysis above, and any other analysis you would like to preform, describe how precipitation in the past five years has changed with respect to the historical climatology of the region. If the observed patterns continue, how do you expect the climate in this area to change in the future? Describe any *trends* or patterns that seem telling.
...
8) *(2 points)* Create two plots or figures that to present to stakeholders in a report in order to help focus remediation efforts for these different sites. Write a short statement with your recommendations or concerns to go alongisde the figures.
...
12) Build a linear model to predict production based on irradiation and temperature. Evaluate the model R^2 and p-value.

13) Using your model, estimate average production if temperature was 5 degrees C higher.

14) Assess whether degradation seems to have an impact on production. Also report the cumulative degradation in the solar panels over the 30 day time period as a percent. 

15) Convert the average daily production from kWh to gigajoule (GJ), given the conversion factor 1 kWh = 0.0036 GJ. Report the total production in GJ over this 30 day time period. 

16) If the solar farm covers 5 km2, calculate the total kW generated over the 30 days
