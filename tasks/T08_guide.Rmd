...
In the task, you will need to learn how to reclassify the values of raster and combine layers together, similar to a suitability analysis. There is more than one way to do so.

First, let's adjust the slope layer we created of Luxembourg to be a bit steeper, then we will find all slope angles greater than 20º. We can use raster algebra, specifically, boolean operations to determine where this condition is met. The result of a boolean operation on a raster is a new TRUE (1) or FALSE (0) raster showing pixels that met the condition:
...
plot(hill, col=grey(0:100/100), legend=FALSE, mar=plot_mar, axes=FALSE, main = "Slopes > 40º")
...
**Use the T08 doc to get started on this task!**
